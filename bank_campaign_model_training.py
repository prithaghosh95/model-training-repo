...
bucket = storage_client.bucket("mlops_pritha")
...
def load_model_artifact(file_name):
    blob = bucket.blob(f"bank_campaign_artifact/{file_name}")
    blob.download_to_filename(file_name)
    return load(file_name)

def write_metrics_to_bigquery(algo_name, training_time, model_metrics):
    client = bigquery.Client()
    table_id = "silken-realm-437403-i1.mlops_pritha.bank_campaign_model_metrics"
...
def main():
    input_data_path = "gs://mlops_pritha/bank_campaign_data/bank-additional.csv"
    model_name='xgboost'
